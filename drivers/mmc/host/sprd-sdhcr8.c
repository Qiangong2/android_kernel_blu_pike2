/*
 * linux/drivers/mmc/host/sprd-sdhcr8.c - Secure Digital Host Controller
 * Interface driver
 *
...
#include "sprd-sdhcr8.h"

extern unsigned int sprd_get_iwhale2_chipid(void);

#define DRIVER_NAME "sprd_sdhcr8"
...
static int sprd_get_a0_delay_value(struct platform_device *pdev)
{
	int ret = 0;
	u32 dly_vl[4];
	...
	ret = of_property_read_u32_array(np, "sprd,a0_mmchs-dly", dly_vl, 4);
	if (!ret)
		timing_dly->mmchs_dly = SPRD_SDHC_DLY_TIMING(dly_vl[0],
					dly_vl[1], dly_vl[2], dly_vl[3]);

	ret = of_property_read_u32_array(np, "sprd,a0_sdr50-dly", dly_vl, 4);
	if (!ret)
		timing_dly->sdr50_dly = SPRD_SDHC_DLY_TIMING(dly_vl[0],
					dly_vl[1], dly_vl[2], dly_vl[3]);

	ret = of_property_read_u32_array(np, "sprd,a0_sdr104-dly", dly_vl, 4);
	if (!ret)
		timing_dly->sdr104_dly = SPRD_SDHC_DLY_TIMING(dly_vl[0],
					dly_vl[1], dly_vl[2], dly_vl[3]);

	ret = of_property_read_u32_array(np, "sprd,a0_ddr52-dly", dly_vl, 4);
	if (!ret)
		timing_dly->ddr52_dly = SPRD_SDHC_DLY_TIMING(dly_vl[0],
					dly_vl[1], dly_vl[2], dly_vl[3]);

	ret = of_property_read_u32_array(np, "sprd,a0_hs200-dly", dly_vl, 4);
	if (!ret)
		timing_dly->hs200_dly = SPRD_SDHC_DLY_TIMING(dly_vl[0],
					dly_vl[1], dly_vl[2], dly_vl[3]);

	ret = of_property_read_u32_array(np, "sprd,a0_hs400-dly", dly_vl, 4);
	if (!ret)
		timing_dly->hs400_dly = SPRD_SDHC_DLY_TIMING(dly_vl[0],
					dly_vl[1], dly_vl[2], dly_vl[3]);

	ret = of_property_read_u32_array(np, "sprd,a0_hs400es-dly", dly_vl, 4);
	if (!ret)
...
static void sprd_sdhc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->clock == 0) {
		...
		clkchg_flag = 1;
		mmiowb();
		sprd_sdhcr8_clk_set_and_on(host, div);
		...
		struct sprd_sdhc_host *host)
{
	...
	/* r8p0 need only one clock source */
	...
	host->sdio_1x_ckg = devm_clk_get(&pdev->dev, "ckg_1x_eb");
	if (IS_ERR(host->sdio_1x_ckg)) {
		dev_warn(&pdev->dev,
			"sdio can't get ckg_1x eb in dts maybe default on\n");
...
#if defined(CONFIG_SOC_IWHALE2)
	if (sprd_get_iwhale2_chipid() == 1) {
		if (sprd_get_delay_value(pdev))
			goto out;
	} else {
		if (sprd_get_a0_delay_value(pdev))
			goto out;
	}
#else
	if (sprd_get_delay_value(pdev))
...
static int sprd_sdhc_runtime_resume(struct device *dev)
{
	...
	if (host->ios.clock) {
		sprd_sdhc_sd_clk_off(host);
		sprd_sdhcr8_clk_set_and_on(host,
