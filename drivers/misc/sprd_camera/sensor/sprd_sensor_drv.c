			...
			const struct i2c_device_id *id)
{
	...
	#ifdef WW6_DRV_CAMERA_MAIN_SUB_EXCHANGE
...
int sprd_sensor_set_voltage_by_gpio(int sensor_id, unsigned int val, int type)
{
	...
	if (gpio_is_valid(gpio_id)) {
		struct device *dev = &p_dev->i2c_info->dev;

		ret = devm_gpio_request(dev, gpio_id,
			sprd_sensor_gpio_names[type]);
		if (ret)
			goto exit;
	...
	if (set_mclk == 0) {
		if (p_dev->mclk_freq) {
			if (p_dev->sensor_clk) {
				...
				#ifdef WW6_THE_FIRST_TIME_OPEN_SAMREA_MCLK_24MHZ
...
int sprd_sensor_set_pd_level(int sensor_id, int power_level)
{
	...
	if (gpio_is_valid(pwn_gpio_id)) {
		...
		ret = devm_gpio_request(dev, pwn_gpio_id,
			sprd_sensor_gpio_names[SPRD_SENSOR_PWN_GPIO_TAG_E]);

		if (power_level == 0)
			ret = gpio_direction_output(pwn_gpio_id, 0);
		else
			ret = gpio_direction_output(pwn_gpio_id, 1);
		devm_gpio_free(dev, pwn_gpio_id);
...
int sprd_sensor_reset(int sensor_id, unsigned int level, unsigned int width)
{
	...
	if (gpio_is_valid(reset_gpio_id)) {
		...
		ret = devm_gpio_request(dev, reset_gpio_id,
			sprd_sensor_gpio_names[SPRD_SENSOR_RESET_GPIO_TAG_E]);

		ret = gpio_direction_output(reset_gpio_id, level);
		gpio_set_value(reset_gpio_id, level);
		mdelay(width);
		gpio_set_value(reset_gpio_id, !level);
		devm_gpio_free(dev, reset_gpio_id);
	}
...
int sprd_sensor_set_mipi_level(int sensor_id, uint32_t plus_level)
{
	...
	if (gpio_is_valid(mipi_gpio_id)) {
		...
		ret = devm_gpio_request(dev, mipi_switch_en,
			sprd_sensor_gpio_names[
				SPRD_SENSOR_MIPI_SWITCH_EN_GPIO_TAG_E]);
		ret = gpio_direction_output(mipi_switch_en, 0);
		gpio_set_value(mipi_switch_en, 0);
		devm_gpio_free(dev, mipi_switch_en);

		ret = devm_gpio_request(dev, mipi_gpio_id,
			sprd_sensor_gpio_names[
		...
		devm_gpio_free(dev, mipi_gpio_id);
