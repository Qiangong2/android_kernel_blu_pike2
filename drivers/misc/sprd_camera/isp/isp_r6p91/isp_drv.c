...
int isp_set_statis_buf(void *isp_pipe_dev_handle)
{
	...
	statis_module->img_statis_buf.ion =
	  (void *)sprd_ion_client_get(statis_module->img_statis_buf.dev_fd);
		     ...
		     unsigned int cmd, unsigned long param)
{
	...
	case SPRD_ISP_IO_CAPABILITY:
		...
		break;
	default:
		pr_err("isp_ioctl: cmd is unsupported, cmd = %x\n",
			(int32_t)cmd);
		ret = -EFAULT;
		goto exit;
	}
...
int sprd_isp_unmap_buf(void *isp_pipe_dev_handle)
{
	...
	pr_info("lsc fd 0x%x, iova 0x%lx, fetch fd 0x%x, iova 0x%lx,store fd 0x%x, iova 0x%lx\n",
		isp_k_param->lsc_pfinfo.mfd[0],
		isp_k_param->lsc_pfinfo.iova[0],
		isp_k_param->fetch_pfinfo.mfd[0],
		isp_k_param->fetch_pfinfo.iova[0],
		isp_k_param->store_pfinfo.mfd[0],
