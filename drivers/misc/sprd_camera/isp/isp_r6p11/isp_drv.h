...
struct isp_frm_queue {
	const char *owner;
	struct camera_frame frm_array[ISP_FRM_QUEUE_LENGTH + 1];
	unsigned int valid_cnt;
	spinlock_t lock;
};
...
struct isp_sc_coeff {
	...
	struct isp_offline_desc store_cce;
};
...
struct isp_pipe_dev {
	...
	/* use this flag when video path is enabled with capture path */
