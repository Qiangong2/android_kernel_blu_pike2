	...
	struct camera_frame *frame)
{
	...
	if (queue->valid_cnt >= (isp_frm_queue_len + 1)) {
		pr_err("fail to enque,over flow,owner:%s type:%d\n",
	...
	struct camera_frame *frame)
{
	...
	if (queue->valid_cnt == 0) {
		pr_err("fail to deque,under flow %s %pS\n",
	...
	struct camera_frame *frame)
{
	...
	if (queue->read != queue->write) {
		*frame = *queue->read++;
		...
		pr_debug("read buf queue type:%x fid:%d index:%x\n",
			frame->type, frame->fid, queue->r_index);
	} else {
		ret = -EAGAIN;
	...
	struct camera_frame *frame)
{
	...
	pr_debug("write buf queue, cb:%pS\n", __builtin_return_address(0));
	spin_lock_irqsave(&queue->lock, flags);
	...
	*queue->write++ = *frame;
	...
	pr_debug("write buf queue type:%x fid:%d index:%x\n",
		 frame->type, frame->fid, queue->w_index);
	...
	uint8_t off_type, struct camera_frame *out_frame)
{
	...
	if (isp_buf_queue_read(p_buf_queue, &frame) == 0 &&
		...
		*out_frame = frame;
		...
		pr_err("fail to read isp offline buf que\n");
		return -EFAULT;
	}
