...
static int reservedframe_iommu_ctrl(void *isp_handle, bool get);
/*======================ISP debug control=====================*/
/*
...
static void isp_irq_enable(struct isp_pipe_dev *dev)
{
	unsigned int idx = dev->com_idx;
...
static void isp_irq_clear(struct isp_pipe_dev *dev)
{
	unsigned int idx = dev->com_idx;
	enum isp_id iid = ISP_GET_IID(dev->com_idx);
	enum isp_scene_id sid = ISP_SCENE_PRE;
...
static int sprd_isp_stop_nolock(void *isp_handle, int is_post_stop)
{
	...
	isp_statis_frm_queue_clear(&statis_module->hist_statis_frm_queue);
	isp_statis_frm_queue_clear(&statis_module->hist2_statis_frm_queue);
...
int sprd_isp_stop(void *isp_handle, int is_irq)
{
	...
	dev = (struct isp_pipe_dev *)isp_handle;
	iid = ISP_GET_IID(dev->com_idx);
	iid_other = iid == ISP_ID_0 ? ISP_ID_1 : ISP_ID_0;
	...
	reservedframe_iommu_ctrl(isp_handle, false);
	pr_info("isp stop\n");
...
int sprd_isp_start_pipeline_bin(void *handle, unsigned int cap_flag)
{
	...
	if (buf_desc->zsl_queue.valid_cnt > ISP_OFF_CONSUMER_Q_SIZE) {
...
int sprd_isp_start_pipeline_full(void *handle, unsigned int cap_flag)
{
	...
	if (cap_flag == DCAM_CAPTURE_NONE)
		...
		/*
		 * Bypass capture process here when isp video
		 * path is valid for SharkLE. Or code flow
		 * will stuck in waiting new zsl frame from full path
		 * which is because the legacy upper layer code
		 * of sharkl2 send ioctl SPRD_IMG_IO_START_CAPTURE
		 * to speed-up recording capture process.
		 */
		if (unlikely(module->isp_path[ISP_SCL_VID].valid)) {
			/*
			  * temporary fix to enable video path with capture
			  * path to pass cts of video snapshot.
			  * This fix will not affect normal functionality
			  */
			ret = isp_path_cap_with_vid_set_next_frm(dev);
			if (ret) {
				pr_info("failed to start video snapshot\n");
			} else {
				pr_info("video with cap enabled\n");
				dev->cap_temp = 1;
			}
			goto normal_exit;
		}
	...
	if (dev->cap_flag == DCAM_CAPTURE_START_HDR)
		...
		max_zsl_num = ISP_OFF_CONSUMER_Q_SIZE;
	...
	if (isp_frame_dequeue(&buf_desc->zsl_queue, &frame)) {
		pr_err("fail to dequeue offline_buf full_path frame\n");
		ret = -EPERM;
		goto err_exit;
	}
...
static void do_shadow_clr(struct isp_pipe_dev *dev, unsigned int idx)
{
	...
	if (pre_path->valid || cap_path->valid) {
		...
		pr_debug("shadow clear for pre/cap path\n");
	} else {
		ISP_REG_WR(idx,
			(ISP_STORE_PRE_CAP_BASE + ISP_STORE_SHADOW_CLR),
			0x0);
	...
	if (vid_path->valid) {
		ISP_REG_WR(idx,
			   ...
			   0x1);
		pr_debug("shadow clear for vid path\n");
	} else {
		ISP_REG_WR(idx,
			(ISP_STORE_VID_BASE + ISP_STORE_SHADOW_CLR),
			0x0);
	...
	struct isp_raw_proc_info *raw_cap)
{
	...
	ret = set_isp_path_cfg((void *)dev, ISP_PATH_IDX_CAP,
		ISP_PATH_INPUT_FORMAT, &is_loose);
	if (ret)
		return ret;
			       ...
			       struct camera_size *out_size)
{
	...
	struct isp_offline_desc *store_cce = NULL;
	struct isp_sc_array *scl_array = NULL;
	enum isp_id iid = 0;
...
static int iommu_get_addr_by_path(struct isp_module *module,
	enum isp_scl_id path_id)
{
	...
	if (reserved_frame != NULL && reserved_frame->pfinfo.iova[0] == 0) {
		rtn = pfiommu_get_addr(&reserved_frame->pfinfo);
		if (rtn) {
			pr_err("ISP: get reserved buffer addr failed!\n");
...
static void iommu_release_addr_by_path(struct isp_module *module,
	enum isp_scl_id path_id)
{
	struct camera_frame *reserved_frame = NULL;
	...
	if (reserved_frame != NULL) {
		if (path_id == ISP_SCL_PRE || path_id == ISP_SCL_CAP)
			pfiommu_free_addr_with_id(&reserved_frame->pfinfo,
						  ISP_IOMMU_CH_AW,
			...
			pfiommu_free_addr_with_id(&reserved_frame->pfinfo,
						  ISP_IOMMU_CH_AW,
						  AW_ID_STORE_VID_YUV);
		reserved_frame->pfinfo.iova[0] = 0;
		reserved_frame->pfinfo.iova[1] = 0;
	}
}
...
static int reservedframe_iommu_ctrl(void *isp_handle, bool get)
{
	...
	if (module == NULL) {
		pr_err("module zero pointer\n");
		return -EFAULT;
	}

	if (get) {
		iommu_get_addr_by_path(module, ISP_SCL_PRE);
		iommu_get_addr_by_path(module, ISP_SCL_VID);
		iommu_get_addr_by_path(module, ISP_SCL_CAP);
	} else {
		iommu_release_addr_by_path(module, ISP_SCL_PRE);
		iommu_release_addr_by_path(module, ISP_SCL_VID);
		iommu_release_addr_by_path(module, ISP_SCL_CAP);
	}
...
int sprd_isp_start(void *isp_handle, struct camera_frame *frame)
{
	...
	reservedframe_iommu_ctrl(isp_handle, true);
	...
	enum isp_cfg_id id, void *param)
{
	...
	case ISP_PATH_INPUT_FORMAT:
		is_loose = *(unsigned int *)param;

		format = is_loose ?
			ISP_FETCH_RAW_10 :
			ISP_FETCH_CSI2_RAW_10;
		off_desc->buf_desc_bin.output_format = format;
		off_desc->buf_desc_full.output_format = format;
...
int sprd_isp_dev_init(void **isp_pipe_dev_handle, enum isp_id iid)
{
	...
	ret = isp_statis_queue_init(
		&dev->statis_module_info.hist2_statis_queue);
