...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "ISP_INT: %s " fmt, __func__
				...
				struct isp_statis_buf *node)
{
	...
	unsigned long FV0_LOW = ISP_RGB_AFM_ENHANCE_FV0_0_LOW;
...
static void isp_cap_with_video(void *isp_handle)
{
	int ret = 0;
	enum isp_id iid;
	struct isp_pipe_dev *dev = NULL;
	...
	if (ret) {
		pr_err("fail to dequeue frame queue,path %d\n", path_id);
		return;
	}
	frame.width = path->dst.w;
	frame.height = path->dst.h;
	...
	void *isp_handle)
{
	...
	if (path->need_wait) {
		...
		if (frame.pfinfo.mfd[0] !=
			...
			if (user_func)
				(*user_func)(&frame, data);
			if (unlikely(dev->cap_temp == 1))
				isp_cap_with_video(isp_handle);
		} else
