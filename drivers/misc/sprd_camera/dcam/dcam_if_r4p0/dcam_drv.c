	...
	void *unmap_func, void *param)
{
	...
	unsigned int flag = 0;
	enum dcam_id idx_other = (idx == DCAM_ID_0) ? DCAM_ID_1 : DCAM_ID_0;
...
int set_dcam_pdaf_path_cfg(enum dcam_id idx, enum dcam_cfg_id id, void *param)
{
	...
	case DCAM_PATH_OUTPUT_RESERVED_ADDR:
		...
		if (DCAM_YUV_ADDR_INVALID(p_addr->yaddr,
			...
			/* save the info to pdaf_reserved_frame */
			memcpy(&(module->pdaf_reserved_frame), frame,
					sizeof(struct camera_frame));
...
#if 0
	/*
	 * disable QoS setting because 9820e has been verified for months
	 * without any bandwidth problem
	 */
