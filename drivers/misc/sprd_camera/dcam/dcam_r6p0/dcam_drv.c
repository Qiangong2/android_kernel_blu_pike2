...
static struct wake_lock         dcam_wakelock;
			    ...
			    uint32_t *channel_id)
{
	...
	pr_info("isp_tool %d, need_interp %d, work {%d, %d, %d}\n",
	...
	if (path->input_rect.w > (path->output_size.w * d_max *  (1<<f_max)) ||
		...
		if (path->input_rect.w > path->output_size.w * d_max) {
			...
			path->input_rect.w = (path->input_rect.w) &
						    ~(align_size-1);
			path->input_rect.x = (path->input_rect.x) &
						    ~(align_size-1);
			path->sc_input_size.w = path->input_rect.w >>
		...
		if (path->input_rect.h > path->output_size.h * d_max) {
			...
			path->input_rect.h = (path->input_rect.h) &
							~(align_size-1);
			path->input_rect.y = (path->input_rect.y) &
							~(align_size-1);
			path->sc_input_size.h = path->input_rect.h >>
...
int32_t dcam_reset(enum dcam_rst_mode reset_mode)
{
	...
	case DCAM_RST_ALL:
		regmap_update_bits(cam_ahb_gpr,
				   REG_MM_AHB_AHB_RST,
				   BIT_MM_AHB_DCAM_SOFT_RST |
				   BIT_MM_AHB_ISP_LOG_SOFT_RST|
				   BIT_MM_AHB_ISP_CFG_SOFT_RST|
				   BIT_MM_AHB_DCAM_CAM0_SOFT_RST |
				   BIT_MM_AHB_DCAM_CAM1_SOFT_RST |
				   BIT_MM_AHB_DCAM_CAM2_SOFT_RST |
