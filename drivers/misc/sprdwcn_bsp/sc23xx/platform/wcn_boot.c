...
static int marlin_write_cali_data(void)
{
	...
	ret = sprdwcn_bus_reg_read(WIFI_REG, &reg_val, 4);
	if (ret < 0) {
		WCN_ERR("marlin read ahb0 fail:%d\n", ret);
		return ret;
	}
	reg_val |= (0x1<<5);
	ret = sprdwcn_bus_reg_write(WIFI_REG, &reg_val, 4);
	if (ret < 0) {
		WCN_ERR("marlin write ahb0 error:%d\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		WCN_ERR("marlin write cali data error:%d\n", ret);
		return ret;
	}

	/* write this flag to notify cp start calibrate */
	reg_val = 0xaaaaaaaa;
	ret = sprdwcn_bus_reg_write(CALI_REG, &reg_val, 4);
	if (ret < 0) {
		WCN_ERR("marlin write cali tag error:%d\n", ret);
		return ret;
	}
	WCN_INFO("marlin send cali data finish\n");
...
static void check_need_loopcheck_or_not(void)
{
	if ((test_bit(MARLIN_BLUETOOTH, &marlin_dev->power_state) +
		...
		(marlin_dev->loopcheck_status_change == 0)) {
...
int find_firmware_path(void)
{
	...
	memcpy(&GNSS_FIRMWARE_PATH[pre_len], "wcnfdl", strlen("wcnfdl"));
	GNSS_FIRMWARE_PATH[pre_len + strlen("wcnfdl")] = '\0';
