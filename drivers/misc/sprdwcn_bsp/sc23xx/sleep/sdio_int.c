...
int pub_int_handle_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (sdio_power_notify)
			...
			wake_unlock(&(sdio_int.pub_int_wakelock));
...
static irqreturn_t pub_int_isr(int irq, void *para)
{
	...
	if (sdio_power_notify)
		...
		wake_lock(&(sdio_int.pub_int_wakelock));
		...
		PUB_INT_ISR isr_handler)
{
	...
	SLP_MGR_ERR("0X%x pub_int_RegCb", bit);
...
int sdio_pub_int_init(int irq)
{
	...
	wake_lock_init(&(sdio_int.pub_int_wakelock), WAKE_LOCK_SUSPEND,
		"pub_int_wakelock");
...
int sdio_pub_int_deinit(void)
{
	...
	wake_lock_destroy(&(sdio_int.pub_int_wakelock));
