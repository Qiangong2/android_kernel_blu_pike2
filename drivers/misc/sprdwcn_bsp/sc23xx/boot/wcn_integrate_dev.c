...
{
	wcn_boot_init();
	pr_info("%s global source init finish!\n", __func__);
}
...
static int wcn_codes_debug_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		case WCN_PRINT_INFO:
			pr_info("wcn cali[data=%p flag=%p] efuse=%p status=%p gnss=%p\n",
...
static void wcn_config_ctrlreg(struct wcn_device *wcn_dev, u32 start, u32 end)
{
	...
	for (i = start; i < end; i++) {
		...
		pr_info("%s:ctrl_reg[%d]=0x%x,read=0x%x, set=%x\n",
			__func__, i, reg_read, val,
		...
		pr_info("%s:rmap[%d]=%p,ctrl_reg[i]=\n",
			__func__, type, wcn_dev->rmap[type]);
	...
	struct wcn_device *wcn_dev)
{
	...
	if (wcn_platform_chip_type() == WCN_PLATFORM_TYPE_SHARKLE) {
		...
		if (IS_ERR(wcn_dev->rmap[REGMAP_PUB_APB])) {
			pr_err("failed to find sprd,syscon-ap-pub-apb\n");
			return -EINVAL;
		}
	}
	...
	if (IS_ERR(wcn_dev->rmap[REGMAP_ANLG_WRAP_WCN])) {
		pr_err("failed to find sprd,anlg-wrap-wcn\n");
		return -EINVAL;
	}
	...
	if (wcn_platform_chip_type() == WCN_PLATFORM_TYPE_SHARKLE) {
		...
		if (IS_ERR(wcn_dev->rmap[REGMAP_ANLG_PHY_G6])) {
			pr_err("failed to find sprd,anlg-phy-g6\n");
			return -EINVAL;
		}
	}
	...
	if (wcn_platform_chip_type() == WCN_PLATFORM_TYPE_SHARKL3) {
		...
		if (IS_ERR(wcn_dev->rmap[REGMAP_WCN_REG])) {
			pr_err("%s:failed to find sprd,wcn-reg\n", __func__);
			return -EINVAL;
		}

		pr_info("%s:succ to find sprd,wcn-reg for SharkL3 %p\n",
			   __func__, wcn_dev->rmap[REGMAP_WCN_REG]);
	}

	ret = of_property_read_u32(np, "sprd,ctrl-probe-num",
				   &wcn_dev->ctrl_probe_num);
	if (ret) {
		pr_err("failed to find sprd,ctrl-probe-num\n");
		return -EINVAL;
	}
	...
	if (cr_num > REG_CTRL_CNT_MAX) {
		pr_err("%s:DTS config err. cr_num=%d\n", __func__, cr_num);
		return -EINVAL;
	}
	...
	for (i = 0; i < cr_num; i++)
		pr_info("%s: ctrl_mask[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_mask[i]);
	...
	if (ret)
		return -EINVAL;
	for (i = 0; i < cr_num; i++)
		pr_info("%s: ctrl_value[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_value[i]);
	...
	if (ret)
		return -EINVAL;
	for (i = 0; i < cr_num; i++)
		pr_info("%s: ctrl_rw_offset[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_rw_offset[i]);
	...
	if (ret)
		return -EINVAL;
	for (i = 0; i < cr_num; i++)
		pr_info("%s: ctrl_us_delay[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_us_delay[i]);
	...
	for (i = 0; i < cr_num; i++)
		pr_info("%s: ctrl_type[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_type[i]);
	...
	for (i = 0; i < cr_num; i++) {
		pr_info("%s: ctrl_shutdown_reg[%d] = 0x%x\n",
			__func__, i, wcn_dev->ctrl_shutdown_reg[i]);
	}
	...
	if (ret)
		return -EINVAL;
	for (i = 0; i < cr_num; i++) {
		pr_info("%s: ctrl_shutdown_mask[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_shutdown_mask[i]);
	}
	...
	if (ret)
		return -EINVAL;
	for (i = 0; i < cr_num; i++) {
		pr_info("%s: ctrl_shutdown_value[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_shutdown_value[i]);
	}
	...
	if (ret)
		return -EINVAL;
	for (i = 0; i < cr_num; i++) {
		pr_info("%s: ctrl_shutdown_rw_offset[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_shutdown_rw_offset[i]);
	}
	...
	if (ret)
		return -EINVAL;
	for (i = 0; i < cr_num; i++) {
		pr_info("%s: ctrl_shutdown_us_delay[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_shutdown_us_delay[i]);
	}
	...
	for (i = 0; i < cr_num; i++)
		pr_info("%s: ctrl_shutdown_type[%d] = 0x%08x\n",
			__func__, i, wcn_dev->ctrl_shutdown_type[i]);
	...
	if (strcmp(wcn_dev->name, WCN_MARLIN_DEV_NAME) == 0) {
		...
		if (IS_ERR(wcn_dev->vddwifipa)) {
			pr_err("Get regulator of vddwifipa error!\n");
			return -EINVAL;
		}
	}
	...
	pr_info("%s: cp base = %llu, size = 0x%x\n",
			__func__, (u64)wcn_dev->base_addr, wcn_dev->maxsz);
	...
	ret = of_property_read_u32_index(np,
					  ...
					  0,
					  &wcn_dev->file_length);
	pr_info("%s:wcn_dev->file_length:%d\n", __func__, wcn_dev->file_length);
	if (ret)
		return -EINVAL;
				...
				loff_t *ppos)
{
	...
	if (copy_from_user(&str[0], buf, WCN_PROC_FILE_LENGTH_MAX) == 0) {
		...
		pr_err("%s: copy_from_user too length %s!\n", __func__, buf);
		return -EINVAL;
	}
