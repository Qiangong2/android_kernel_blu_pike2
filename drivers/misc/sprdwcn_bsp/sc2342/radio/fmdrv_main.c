...
#define FM_CMD_PARSE_LEN		(32)
#define MAX_FREQUENCY_LMT		(1080)
...
void parse_at_fm_cmd(unsigned int *freq_found)
{
	...
	int len = 0;
	char *cur_ptr;
	char num_str[FM_CMD_PARSE_LEN] = {0};
	int result = 0;

	cur_ptr = fmdev->read_buf;
	read_flag = 0;
	for (i = 0; i < FM_CMD_PARSE_LEN && cur_ptr[i] != '\0'; i++) {
		if (cur_ptr[i] == ',')
			comma_cou++;
		if (comma_cou == 3) {
			comma_cou = 0;
	...
	for (i = 0, cmdstart++; cmdstart < FM_CMD_PARSE_LEN &&
		cur_ptr[cmdstart] != '\0' &&
...
unsigned int fm_rx_cback(void *addr, unsigned int len, unsigned int fifo_id)
{
	...
	pr_info("(fmdrv) %s(): addr =0x%p,len =%d, fifo_id =%d\n", __func__,
		(unsigned int *)addr, len, fifo_id);
#ifdef FM_DUMP_DATA
	dump_rx_data(buf, len);
#endif
	if (fmdev != NULL) {
...
{   struct fm_tune_parm parm;
	...
	/* para is between 875~1080 */
	if (parm.freq <= MAX_FREQUENCY_LMT)
		parm.freq *= 10;
#ifdef RDS_DEBUG
	...
	pr_info("(fmdrv) fm tune have finshed!!status =0,RSSI=%d\n"
