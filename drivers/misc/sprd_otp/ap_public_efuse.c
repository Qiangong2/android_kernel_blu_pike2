			 ...
			 struct device_attribute *attr, char *buf)
{
	...
	for (uidnum = ap_efuse->uid_start[0]; uidnum <= ap_efuse->uid_end[0];
	     uidnum++) {
		p += sprintf(p, "[%02d] %08x\n", uidnum,
			     sprd_efuse_double_read(uidnum,
						    ap_efuse->uid_start[1]));
	}
...
static ssize_t sc7731e_kaios_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	u32 block = 43;

	u32 data = sprd_efuse_double_read(block, 1);
	return sprintf(buf, "%x\n", data);
}
static DEVICE_ATTR_RO(sc7731e_kaios);
