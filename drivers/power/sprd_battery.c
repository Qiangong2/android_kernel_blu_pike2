...
{
	int i = 0, compare = 0;
	int last_temp = sprdbat_data->bat_info.last_temp;
	int cur_temp = sprdbat_data->bat_info.cur_temp;

	for (i = 0; i < size; i++) {
		SPRDBAT_DEBUG("data=%d, buf[%d].x=%d, buf[%d].w=%d\n",
			data, i, buf[i].x, i, buf[i].w);
		if (((last_temp > cur_temp) && (cur_temp >= NORMAL_TEMP)) ||
			((last_temp < cur_temp) && (cur_temp < NORMAL_TEMP)))
			compare = buf[i].w;
		else
			compare = buf[i].x;
		if (data <= compare)
...
static void sprdbat_temp_monitor(void)
{
	...
	if (sprdbat_data->bat_info.last_temp !=
					...
					const char *buf, size_t count)
{
	...
	if (off != TEMP_TAB) {
		if (kstrtoul(buf, 10, &set_value))
			return -EINVAL;
		pr_info("battery calibrate value %lu\n", set_value);
	}
	...
	case ADJUST_CUR_MAX:
		if (set_value > sprdbat_data->bat_info.chg_current_type) {
			SPRDBAT_DEBUG("set_value over range %lu\n",
				      set_value);
	...
	case CHG_COOL_STATE:
		...
		SPRDBAT_DEBUG("set_cur_state state=%lu\n", set_value);
...
static int plugin_callback(void)
{
	...
	if (sprdchg_timer_op->timer_enable)
		sprdchg_timer_op->timer_enable(sprdbat_data->
...
static void sprdbat_fault_monitor(void)
{
	...
	if (chg_fault & SPRDBAT_CHG_END_BAT_OVP_BIT) {
		if (sprdbat_data->sprdbat_vbat_ovp_cnt < VBAT_OVP_THRESHOLD) {
...
*/
	...
	if (sprdbat_data->bat_info.vbat_vol <=
	    ...
	    SPRDBAT_SHUTDOWN_OFSSET) {
		fgu_capacity = 0;
        SPRDBAT_DEBUG("soft uvlo, shutdown by kernel status full\n");
...
static void sprdbat_battery_works(struct work_struct *work)
{
	...
	queue_delayed_work(system_power_efficient_wq,
			&sprdbat_data->battery_work, 15 * HZ);
}
...
int __weak sprdbat_get_bat_id(struct platform_device *pdev)
{
	...
	channel_temp = iio_channel_get(&pdev->dev, "adc_temp");
	if (IS_ERR(channel_temp))
		return PTR_ERR(channel_temp);
	ntc_vol = sprdchg_read_ntc_vol(channel_temp);
	SPRDBAT_DEBUG("%s ntc_vol:%d\n", __func__, ntc_vol);

	/* if ntc voltage value is over 50mv, it is battery 1 */
	if (ntc_vol < BAT_NTC_THRESHOLD)
		ret = 1;
	else
		ret = 0;
	iio_channel_release(channel_temp);
