...
void sprd_iommu_set_list(struct sprd_iommu_dev *iommu_dev)
{
	if (iommu_dev == NULL) {
...
bool sprd_iommu_target_buf(struct sprd_iommu_dev *iommu_dev,
						void *buf_addr,
						unsigned long *iova_addr)
{
	int index = 0;
...
bool sprd_iommu_target_iova_find_buf(struct sprd_iommu_dev *iommu_dev,
						unsigned long iova_addr,
						size_t iova_size,
						void **buf)
{
...
bool sprd_iommu_target_buf_find_iova(struct sprd_iommu_dev *iommu_dev,
						void *buf_addr,
						size_t iova_size,
						unsigned long *iova_addr)
{
...
bool sprd_iommu_insert_slot(struct sprd_iommu_dev *iommu_dev,
						unsigned long sg_table_addr,
						void *buf_addr,
						unsigned long iova_addr,
						unsigned long iova_size)
...
bool sprd_iommu_clear_sg_iova(struct sprd_iommu_dev *iommu_dev,
			unsigned long sg_addr, unsigned long *iova,
			unsigned long *size)
{
	int index;
	struct sprd_iommu_sg_rec *rec;
...
void sprd_iommu_pool_show(struct sprd_iommu_dev *iommu_dev)
{
	int index;
	struct sprd_iommu_sg_rec *rec;
	...
	if (iommu_dev->map_count > 0)
		for (index = 0; index < SPRD_MAX_SG_CACHED_CNT; index++) {
			rec = &(iommu_dev->sg_pool.slot[index]);
			if (rec->status == SG_SLOT_USED) {
				IOMMU_ERR("Warning! buffer iova 0x%lx "
					"size 0x%lx sg 0x%lx buf %p "
...
int sprd_iommu_attach_device(struct device *dev)
{
	...
	if (!ret) {
		...
		err = -EPERM;
	if (err)
		IOMMU_ERR("%s iommu attach failed!\n", np->name);
	return err;
}
...
int sprd_iommu_unmap(struct device *dev, struct sprd_iommu_unmap_data *data)
{
	...
	if (valid_iova) {
		...
		if (valid_buf) {
			...
			IOMMU_ERR("%s illegal error iova 0x%lx buf %p "
				"size 0x%zx\n",
				iommu_dev->init_data->name,
...
int sprd_iommu_unmap_orphaned(struct sg_table *sg, int id)
{
	...
	iommu_dev = sprd_iommu_list[id].iommu_dev;

	spin_lock_irqsave(&iommu_dev->pgt_lock, flag);

	ret = sprd_iommu_clear_sg_iova(iommu_dev, (unsigned long)sg,
							&iova, &size);
	if (ret) {
		...
		IOMMU_ERR("%s iova leak error, sg %p id %d iova 0x%lx "
			"size 0x%lx\n",
			iommu_dev->init_data->name, sg, id, iova, size);
	} else
		IOMMU_ERR("%s illegal error sg %p id %d\n",
			iommu_dev->init_data->name, sg, id);
				...
				struct sprd_iommu_init_data *pdata)
{
	...
	if (pdata->iommuex_rev) {
		/*sharkl2 no need this*/
		pdata->pgt_base = 0;
	} else {
		pdata->pgt_base = (unsigned long)ioremap_wc(res.start,
			resource_size(&res));
