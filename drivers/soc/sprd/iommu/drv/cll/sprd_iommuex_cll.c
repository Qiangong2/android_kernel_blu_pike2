						...
						sprd_iommu_hdl p_iommu_hdl)
{
	...
	p_iommu_priv->interrupt_reg_addr = p_init_param->interrupt_reg_addr;
...
u32 sprd_iommuex_cll_update(sprd_iommu_hdl  p_iommu_hdl, u32 update)
{
	struct sprd_iommu_widget *p_iommu_data = NULL;
	struct sprd_iommuex_priv *p_iommu_priv = NULL;
	u32 ret = 0;
	...
	iommu_id = p_iommu_priv->iommu_id;
	iommu_type = p_iommu_priv->iommu_type;
	mmu_ex_update(p_iommu_priv->mmu_reg_addr, iommu_id, iommu_type, update);
	return ret;
}
...
#ifdef CONFIG_64BIT
#ifdef CONFIG_ARM64
	__dma_flush_range((void *)(ppn_base +
		start_entry * 4),
		(void *)(ppn_base +
		end_entry * 4));
#else
				...
				struct sprd_iommu_map_param *p_map_param)
{
	...
	for_each_sg(p_map_param->p_sg_table->sgl, sg,
		...
		for (entry_index = 0; entry_index < valid_page_entrys;
			...
			/*sharklj1 should be sub 0x80000000*/
			if (p_iommu_priv->chip == SPRD_IOMMUEX_SHARKLJ1 ||
			    p_iommu_priv->chip == SPRD_IOMMUEX_SHARKLE)
				phy_addr -= 0x80000000;
	...
	*/
	if (iommu_id == IOMMU_EX_ISP) {
		...
		mmu_ex_update(p_iommu_priv->mmu_reg_addr, iommu_id,
			iommu_type, 1);
	/*for sharkle dcam_if r4p0*/
	mmu_ex_frc_copy(p_iommu_priv->mmu_reg_addr, iommu_id, iommu_type);
...
u32 sprd_iommuex_cll_enable(sprd_iommu_hdl p_iommu_hdl)
{
	...
	if (p_iommu_priv->chip == SPRD_IOMMUEX_SHARKLJ1 ||
	    p_iommu_priv->chip == SPRD_IOMMUEX_SHARKLE)
	    fault_page -= 0x80000000;
	mmu_ex_default_ppn(p_iommu_priv->mmu_reg_addr, iommu_id, fault_page);

	if (iommu_id != IOMMU_EX_ISP) {
		if (p_iommu_priv->iommu_type == SPRD_IOMMUEX_SHARKL3 &&
			iommu_id == IOMMU_EX_DISP)
