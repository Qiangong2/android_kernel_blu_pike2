...
#define ANA_RTC_PWR_CTRL                (offset + 0x58)
#define ANA_RTC_PWR_STS                 (offset + 0x5c)
...
static int sprd_rtc_check_power_down(struct device *dev)
{
	...
	pwr_sts = sprd_rtc_read(ANA_RTC_PWR_STS);
	if (pwr_sts == SPRD_RTC_POWER_RESET_VALUE ||
		...
		sprd_rtc_write(ANA_RTC_PWR_CTRL, SPRD_RTC_POWER_STS_CLEAR);

		sprd_rtc_write(ANA_RTC_PWR_CTRL, SPRD_RTC_POWER_STS_VALID);
	}
	return ret;
}
