...
static int sprd_efuse_sharkl2_cpu_gpu_cal(struct sprd_thermal_zone *pzone)
{
	...
	u32 data = sprd_efuse_double_read(pzone->dsen_para->cal_efuse_blk, 1);
#else
	u32 data = 0;
#endif
	int ratio = 1;
...
static int sprd_thm_sensor_temp_read(struct sprd_thermal_zone *pzone, int *temp)
{
	...
	pzone->rawdata =
	    thm_reg_read(pzone->thm_base + pzone->thmreg->thm_temp0_read);
...
static const struct of_device_id ap_thermal_of_match[] = {
	{.compatible = "sprd,r1p0-thm", .data = (void *)SPRD_R1P0_THM},
	{.compatible = "sprd,r2p0-thm", .data = (void *)SPRD_R2P0_THM},
	{},
};
...
static int sprd_ap_thm_probe(struct platform_device *pdev)
{
	...
	int otptemp = 120000;
	int hottemp = 75000;
	int hot2nor_temp = 65000;
	int highoff_temp = 55000;
	int lowoff_temp = 45000;
	int cold_temp = 35000;
