...
#include <linux/mfd/sprd/sc2720_glb.h>
#include "../adf/sprd/drv/disp_notify.h"

/*#define BACKLIGHT_BLTC_DEBUG*/

#ifdef pr_fmt
#undef pr_fmt
#define pr_fmt(__fmt) "[sprd-sc2720-backlight][%20s] "__fmt, __func__
#endif
...
#define REG_BLTC_RG_RGB_V0	(REG_BLTC_BASE + 0x038)
#define REG_BLTC_RG_RGB_V1	(REG_BLTC_BASE + 0x03C)
#define REG_BLTC_RG_RGB_V2	(REG_BLTC_BASE + 0x040)
#define REG_BLTC_RG_RGB_V3	(REG_BLTC_BASE + 0x044)
#define REG_BLTC_WLED_PRESCL    (REG_BLTC_BASE + 0x048)
#define REG_BLTC_WLED_DUTY	(REG_BLTC_BASE + 0x04C)
#define REG_BLTC_WLED_CURVE0	(REG_BLTC_BASE + 0x050)
#define REG_BLTC_WLED_CURVE1	(REG_BLTC_BASE + 0x054)
...
static void sprd_bltc_whtled_off(void)
{
	...
	do {
		...
		regmap_write(sprdbl.bltc_adi, REG_BLTC_WLED_DUTY, 0x0);
...
static void sprd_bltc_whtled_set_brightness(u32 level)
{
	...
	regmap_write(sprdbl.bltc_adi, REG_BLTC_BASE, 0x3333);
	...
	regmap_write(sprdbl.bltc_adi, REG_BLTC_WLED_DUTY, duty);
...
static int sprd_bltc_notify_callback(struct notifier_block *nb,
				unsigned long action, void *data)
{
	if (action == DISP_EVENT_DISPC_RESUME) {
		waitframe = false;
		pr_info("close backlight update before the first frame\n");
	} else if (action == DISP_EVENT_DISPC_WAITFRAME) {
		waitframe = true;
		/* if the brightness is not 0, */
		/* then update backlight immediately. */
		if (sprdbl.bldev->props.brightness)
			sprd_bltc_whtled_set_brightness(sprdbl.led_level);
		pr_info("open backlight update\n");
	}

	return NOTIFY_OK;
}

static int sprd_bltc_notifier_register(struct sprd_bl_devdata *bl_devdata)
{
	bl_devdata->nb.notifier_call = sprd_bltc_notify_callback;

	return disp_notifier_register(&bl_devdata->nb);
}
...
static int whtled_backlight_probe(struct platform_device *pdev)
{
	...
	regmap_write(sprdbl.bltc_adi, REG_BLTC_WLED_PRESCL, 0x0);
	...
	regmap_write(sprdbl.bltc_adi, REG_BLTC_WLED_CURVE0, 0x0);
	...
	regmap_write(sprdbl.bltc_adi, REG_BLTC_WLED_CURVE1, 0x0);

	/*Set default current value*/
	regmap_write(sprdbl.bltc_adi, REG_BLTC_RG_RGB_V0, sprdbl.output_current);
	regmap_write(sprdbl.bltc_adi, REG_BLTC_RG_RGB_V1, sprdbl.output_current);
	regmap_write(sprdbl.bltc_adi, REG_BLTC_RG_RGB_V2, sprdbl.output_current);
	regmap_write(sprdbl.bltc_adi, REG_BLTC_RG_RGB_V3, sprdbl.output_current);

	bldev->ops->update_status(bldev);

	sprd_bltc_notifier_register(&sprdbl);
...
MODULE_AUTHOR("shenhui.sun@unisoc.com");
MODULE_LICENSE("GPL");
