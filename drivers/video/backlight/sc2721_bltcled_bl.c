...
static void sprd_bltc_whtled_get_brightness(void)
{
	...
	int r_curve[2], g_curve[2], b_curve[2];
	...
	regmap_read(sprdbl.bltc_adi, REG_BLTC_R_CURVE0, &r_curve[0]);
	regmap_read(sprdbl.bltc_adi, REG_BLTC_G_CURVE0, &g_curve[0]);
	regmap_read(sprdbl.bltc_adi, REG_BLTC_B_CURVE0, &b_curve[0]);
	regmap_read(sprdbl.bltc_adi, REG_BLTC_R_CURVE1, &r_curve[1]);
	regmap_read(sprdbl.bltc_adi, REG_BLTC_G_CURVE1, &g_curve[1]);
	regmap_read(sprdbl.bltc_adi, REG_BLTC_B_CURVE1, &b_curve[1]);
	...
	pr_info("BLTC: R_CURVE0 = %x, G_CURVE0 = %x, B_CURVE0 = %x\n",
			r_curve[0], g_curve[0], b_curve[0]);
	pr_info("BLTC: R_CURVE1 = %x, G_CURVE1 = %x, B_CURVE1 = %x\n",
			r_curve[1], g_curve[1], b_curve[1]);
}
#endif
...
{
#ifdef WW6_LCD_BL_RESUME_DELAY_TIME
	mdelay(50);//(WW6_LCD_BL_RESUME_DELAY_TIME);	
#endif
