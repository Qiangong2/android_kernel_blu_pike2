...
static int frame_1st = FRAME_1ST_NONE;
...
static void swdispc_disable_te(struct dispc_context *ctx)
{
			...
			struct sprd_restruct_config *config)
{
	...
	if (frame_1st == FRAME_1ST_FLIP) {
		swdispc_wait_te(ctx);
		disp_notifier_call_chain(DISP_EVENT_DISPC_WAITFRAME, NULL);
		frame_1st = FRAME_1ST_NONE;
	}
}
...
{
	if (frame_1st == FRAME_1ST_NONE) {
		disp_notifier_call_chain(DISP_EVENT_DISPC_RESUME, NULL);
		frame_1st = FRAME_1ST_FLIP;
	}
