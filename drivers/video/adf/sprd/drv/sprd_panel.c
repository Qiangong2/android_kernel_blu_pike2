...
static int of_parse_cmds(struct device_node *np,
...
#if defined CONFIG_UDC
extern int udc_get_power_param(struct panel_device  *device);

#if 0
void mipi_timing_from_dt(const struct display_timing *dt,
			 struct panel_info *panel)
{
	struct rgb_timing *t = &panel->rgb_timing;
...
static int sprd_panel_probe(struct platform_device *pdev)
{
	...
	for (index = 0; (index < 2) && panel_id[index]; index++) {
		...
		else
		{
		   panel = kzalloc(sizeof(struct panel_info), GFP_KERNEL);
			if (panel == NULL) {
			//	printk("alloc udc panel failed\n");
			return -ENOMEM;
			}
			pd->panel=panel;
			if	( udc_lcd_create(SEC_LCD0, pd) ){
			//strncpy(zyt_peripherals_LCD_name, panel->lcd_name,sizeof(zyt_peripherals_LCD_name)-1);		
			}

			//mipi_timing_from_dt(&panel->display_timing, panel);
		}			
#else
