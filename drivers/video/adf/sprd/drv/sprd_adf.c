...
static int32_t sprd_adf_suspend(struct sprd_adf_interface *adf_intf)
{
...
static int32_t sprd_adf_resume(struct sprd_adf_interface *adf_intf)
{
	struct sprd_dispc *dispc;
	struct pipe *pipe = (struct pipe *)(adf_intf->data);
	int index;
				...
				struct drm_mode_modeinfo *mode)
{
	...
	if (panel->low_res_simu && panel->simu_width && panel->simu_height) {
		mode->hdisplay = panel->simu_width;
		mode->vdisplay = panel->simu_height;
	} else {
...
static int adf_runtime_suspend(struct device *dev)
{
	...
	int num = adf_context->n_interfaces;
	int i;

	for (i = 0; i < num; i++)
		sprd_adf_suspend(&interfaces[i]);
...
static void sprd_adf_shutdown(struct platform_device *pdev)
{
	...
	/* to avoid adf resume after shutdown */
	dpms_state = true;

	adf_runtime_suspend(&pdev->dev);
}
