...
static struct nand_inst inst_nop = {
	"_inst_nop",
	2, 0, INT_TO | INT_DONE, {
		INST_INOP(2),
		INST_DONE()
		...
		uint32_t num, struct mtd_ecc_stats *ecc_sts, uint32_t mode)
{
	...
	uint32_t sector_num = host->param.nsect_per_page;
...
err:
	pr_err("sprd nand read ecc sts %d\n", ret);
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_OFFSET, 32, 4,
			(void *)(host->mbuf_v + (sector << host->sectshift)), 0x200, 1);
	return ret;
}


static void sprd_nand_int_clr(struct sprd_nand_host *host, uint32_t bit_clear)
{
	writel_relaxed(bit_clear, NFC_INT_REG);
}

static int sprd_nand_wait_finish(struct sprd_nand_host *host)
{
	unsigned int value;
	unsigned int counter = 0;

	while ((counter < NFC_TIMEOUT_VAL)) {
		value = readl_relaxed(NFC_INT_REG);
	...
	writel_relaxed(0xf00, NFC_INT_REG);
...
static unsigned int sprd_nand_data_trans(struct sprd_nand_host *host,
				struct sprd_nand_data_param *param)
{
	uint32_t cfg0 = 0;
	uint32_t i = 0;
	uint32_t cfg1 = 0;
	uint32_t cfg2 = 0;
	uint32_t cfg4 = 0;
	struct nand_inst *inst = &inst_nop;

	cfg0 |= CFG0_SET_ONLY_NAND_MODE;
	cfg4 = readl_relaxed(NFC_CFG4_REG);
	cfg4 &= ~CFG4_ONLY_SEL_MODE;

	for (i = 0; i < inst->cnt; i += 2)
		writel_relaxed((inst->inst[i] | (inst->inst[i + 1] << 16)),
				NFC_INST00_REG + (i << 1));

	if (param->m_sct != 0) {
		writel_relaxed(0x0, NFC_MAIN_ADDRH_REG);
		writel_relaxed((uint32_t)param->m_buf, NFC_MAIN_ADDRL_REG);
	}
	if (param->s_sct != 0) {
		writel_relaxed(0x0, NFC_SPAR_ADDRH_REG);
		writel_relaxed((uint32_t)param->s_buf, NFC_SPAR_ADDRL_REG);
	}
	writel_relaxed(cfg0, NFC_CFG0_REG);
	writel_relaxed(cfg1, NFC_CFG1_REG);
	writel_relaxed(cfg2, NFC_CFG2_REG);
	writel_relaxed(cfg4, NFC_CFG4_REG);
	sprd_nand_int_clr(host,
			INT_STSMCH_CLR | INT_WP_CLR | INT_TO_CLR | INT_DONE_CLR);
	writel_relaxed(CTRL_NFC_CMD_START, NFC_START_REG);
	sprd_nand_wait_finish(host);

	cfg4 = readl_relaxed(NFC_CFG4_REG);
	cfg4 |= CFG4_ONLY_SEL_MODE;

	cfg0 = CFG0_SET_NFC_MODE(CFG0_SET_ONLY_MASTER_MODE) | CFG0_DEF1_MAIN_SPAR_APT(0) | CFG0_SET_WPN;
	if (param->dir)
		cfg0 |= CFG0_SET_NFC_RW;

	if (param->m_sct != 0) {
		cfg0 |= (param->m_sct - 1) << SECTOR_NUM_OFFSET;
		cfg0 |= CFG0_SET_MAIN_USE;
		cfg1 |= (param->m_size - 1);
		writel_relaxed(0x0, NFC_MAIN_ADDRH_REG);
		writel_relaxed((uint32_t)param->m_buf, NFC_MAIN_ADDRL_REG);
	}
	if (param->s_sct != 0) {
		cfg0 |= CFG0_SET_SPAR_USE;
	...
	sprd_nand_int_clr(host,
			INT_STSMCH_CLR | INT_WP_CLR | INT_TO_CLR | INT_DONE_CLR);
	writel_relaxed(CTRL_NFC_ONLY_START, NFC_START_REG);
	sprd_nand_wait_finish(host);
	return 0;
}


static unsigned int ecc_mode_convert(uint32_t mode)
{
	uint32_t mode_m;
...
static void sprd_nand_ecc_ctrl(struct sprd_nand_host *host,
			struct sprd_nand_ecc_param *param, uint32_t dir)
{
	...
	nfc_start |= CTRL_DEF1_ECC_MODE(ecc_mode_convert(param->mode));
	cfg0 = CFG0_SET_NFC_MODE(CFG0_SET_ONLY_ECC_MODE)  | CFG0_DEF1_MAIN_SPAR_APT(0);
	if (dir)
		cfg0 |= CFG0_SET_NFC_RW;
	...
	sprd_nand_int_clr(host, INT_STSMCH_CLR | INT_WP_CLR | INT_TO_CLR | INT_DONE_CLR);
	writel_relaxed(nfc_start | CTRL_NFC_ONLY_START, NFC_START_REG);
	sprd_nand_wait_finish(host);
}

static int sprd_nand_ecc_encode(struct sprd_nfc_base *nfc_base,
		u_char *buf)
{
	struct spl_header_info *spl_header;
	struct sprd_nand_data_param d_param;
	struct sprd_nand_ecc_param ecc_param;
	dma_addr_t ecc_mbuf_p = 0, ecc_sbuf_p = 0;
	void *ecc_mbuf_v = 0, *ecc_sbuf_v = 0;
	struct sprd_nand_host *host = (struct sprd_nand_host *) nfc_base->priv;

	spl_header = (struct spl_header_info *)(buf);
	memset(&d_param, 0, sizeof(d_param));

	ecc_mbuf_v = dma_alloc_coherent(NULL, sizeof(struct spl_header_info),
						&ecc_mbuf_p, GFP_KERNEL);
	if (!ecc_mbuf_v) {
		pr_info("nand:Failed to allocate memory for DMA buffer\n");
		return -ENOMEM;
	}

	ecc_sbuf_v = dma_alloc_coherent(NULL, sizeof(struct spl_header_info),
						&ecc_sbuf_p, GFP_KERNEL);
	if (!ecc_sbuf_v) {
		pr_info("nand:Failed to allocate memory for DMA buffer\n");
		return -ENOMEM;
	}
	memcpy(ecc_mbuf_v, spl_header, sizeof(struct spl_header_info));
	memset(ecc_sbuf_v, 0xff, sizeof(struct spl_header_info));

	/*mtd_ecc_param*/
	ecc_param.mode = 60;
	ecc_param.ecc_num = 1;
	ecc_param.sp_size = 108;
	ecc_param.ecc_pos = 0;
	ecc_param.sinfo_pos = 0;
	ecc_param.sinfo_size = 1;
	ecc_param.m_size = (1<<host->sectshift);
	ecc_param.p_mbuf = (uint8_t *)ecc_mbuf_p;
	ecc_param.p_sbuf = (uint8_t *)ecc_sbuf_p;
	/*nand data_param*/
	d_param.m_buf = (uint8_t *)ecc_mbuf_p;
	d_param.s_buf = (uint8_t *)ecc_sbuf_p;
	d_param.m_sct = ecc_param.ecc_num;
	d_param.s_sct = ecc_param.ecc_num;
	d_param.dir = 1;
	d_param.m_size = ecc_param.m_size;
	d_param.s_size = ecc_param.sp_size;
	...
	sprd_nand_data_trans(host, &d_param);
	sprd_nand_ecc_ctrl(host, &ecc_param, 1);

	d_param.dir = 0;
	d_param.m_sct = 0;
	sprd_nand_data_trans(host, &d_param);

	memcpy(spl_header->ecc_value, ecc_sbuf_v, d_param.s_size);
...
{
	uint32_t reg_nfc_cfg0;

	reg_nfc_cfg0 = readl_relaxed(NFC_CFG0_REG);
	reg_nfc_cfg0 |=CFG0_SET_WPN;
	writel_relaxed(reg_nfc_cfg0, NFC_CFG0_REG);
	udelay(1);
...
static int sprd_nand_ctrl_en(struct sprd_nfc_base *nfc_base, uint32_t en)
{
	...
	if (en) {
		...
		clk_prepare_enable(host->clk_ahb_ecc);
		clk_prepare_enable(host->clk_nand_1x);
		clk_prepare_enable(host->clk_nand_2x);
		...
		struct sprd_nfc_base *base, struct sprd_nand_host *host)
{
	...
	base->nand_ecc_encode = sprd_nand_ecc_encode;
	base->priv = host;
		...
		struct platform_device *pdev, struct sprd_nand_host *host)
{
	...
	host->clk_ahb_ecc = of_clk_get(np, 5);
	if (IS_ERR(host->clk_ahb_ecc)) {
		dev_err(&pdev->dev, "can't get clock:clk_ahb_ecc\n");
		host->clk_ahb_ecc = NULL;
		host->clk_nand_1x = NULL;
		host->clk_nand_2x = NULL;
	} else {
		host->clk_nand_1x = of_clk_get(np, 6);
		host->clk_nand_2x = of_clk_get(np, 7);
	}
...
static int sprd_nand_drv_probe(struct platform_device *pdev)
{
	...
	if (ret)
		return ret;
	if (pdev->dev.of_node) {
		if (!of_property_read_u32(pdev->dev.of_node, "random_mode", &val))
			host->randomizer = val;
		else
			host->randomizer = 0;
	}
