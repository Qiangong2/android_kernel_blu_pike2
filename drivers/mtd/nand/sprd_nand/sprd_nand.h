...
#define CTRL_NFC_ONLY_START         (1 << 3)
...
#define CFG0_SET_AUTO_MODE	0
#define CFG0_SET_ONLY_MASTER_MODE	1
#define CFG0_SET_ONLY_NAND_MODE	2
#define CFG0_SET_ONLY_ECC_MODE	3
...
#define CFG4_ONLY_SEL_MODE				(0x1<<14)
...
#define INT_DONE_RAW				(0x1<<24)
#define INT_STSMCH_CLR				(0x1<<11)
#define INT_WP_CLR					(0x1<<10)
#define INT_TO_CLR					(0x1<<9)
#define INT_DONE_CLR				(0x1<<8)
...
#define MACH_ERASE					((1)|(1<<11)|(1<<22))
/* check IO 0_bit, stop if error */
#define MACH_WRITE					((1)|(1<<11)|(1<<22))
#define MACH_READ					(0)
#define DEF0_MATCH					(0)
...
/**
* struct sprd_nand_ecc_param --ecc operation operands
* @mode:	operation mode
*
* @ecc_num:	max is 8
* @sp_size:	spare part size
* @ecc_pos:	ecc position
* @m_size:	main part size
*/
struct sprd_nand_ecc_param {
	uint8_t mode;
	uint8_t ecc_num;
	uint8_t sp_size;
	uint8_t ecc_pos;
...
/**
* struct spl_header_info --spl nand header
* @magic_num:	0xaa55a5a5
*
* @reserved[70]:	512 - (4 * 2 + 32 + 4 * 2 + 4)
* @spare_size:	spare part sise for one sector
* @ecc_mode:	0--1bit, 1--2bit,2--4bit,3--8bit,4--12bit, 5--16bit, 6--24bit
* @ecc_pos:		ecc postion at spare part
* @sct_per_page:	sector per page
*/
struct spl_header_info {
	uint32_t magic_num;
	uint32_t version;
