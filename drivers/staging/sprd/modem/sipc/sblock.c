...
#define VOLA_SBLOCK_RING volatile struct sblock_ring_header
...
void sblock_put(u8 dst, u8 channel, struct sblock *blk)
{
	...
	ring = sblock->ring;
	poolhd = (VOLA_SBLOCK_RING *)(&ring->header->pool);
...
static int sblock_recover(u8 dst, u8 channel)
{
	...
	ring = sblock->ring;
	ringhd = (VOLA_SBLOCK_RING *)(&ring->header->ring);
	poolhd = (VOLA_SBLOCK_RING *)(&ring->header->pool);
		     ...
		     void (*handler)(int event, void *data), void *data)
{
	...
	ringhd = (VOLA_SBLOCK_RING *)(sblock->smem_virt);
	...
	poolhd = (VOLA_SBLOCK_RING *)(sblock->smem_virt +
...
EXPORT_SYMBOL(sblock_create_ex);
...
int sblock_poll_wait(u8 dst, u8 channel, struct file *filp, poll_table *wait)
{
	struct sblock_mgr *sblock = NULL;
	struct sblock_ring *ring = NULL;
	VOLA_SBLOCK_RING *ringhd = NULL;
...
EXPORT_SYMBOL(sblock_poll_wait);
	...
	for (i = 0; i < SIPC_ID_NR; i++) {
		for (j = 0;  j < SMSG_VALID_CH_NR; j++) {
			...
			if (!sblock)
				continue;
			ring = sblock->ring;
			ringhd = (VOLA_SBLOCK_RING *)(&sblock->ring->
						      header->ring);
			poolhd = (VOLA_SBLOCK_RING *)(&sblock->ring->
						      header->pool);
