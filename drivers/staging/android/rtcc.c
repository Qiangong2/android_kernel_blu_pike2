...
/*
 * The rtcc enable during runtime threshold.
 * This is command line adjustable via setup_rtcc_threshold.
 */
static unsigned int rtcc_threshold;
static bool is_rtcc;
...
#define RTCC_DBG			1
/* default to 768MB */
#define RTCC_ENABLE_THRESHOLD		(768UL)
...
static int rtcc_idle_notify_cnt;
static int rtcc_idle_wakeup_cnt;
...
{
	return global_page_state(NR_INACTIVE_ANON) +
...
#if RTCC_DBG
	pr_info("rtcc grade = %d, swap_top = %ld\n", grade, swap_toplimit);
#endif
...
static int get_reclaim_swappiness(void)
{
	...
	if (unlikely((swap_toplimit - get_swapped_pages())
			<= rtcc_grade[index]))
		return 1;
...
static int rtcc_thread(void *nothing)
{
	...
	for ( ; ; ) {
		...
		if (likely(atomic_read(&krtccd_running) == 1)) {
#if RTCC_DBG
			do_gettimeofday(&tv1);
#endif		/*get_swapped_pages calculate the used swap pages*/
			swap_toplimit = get_swapped_pages()
					+ get_anon_pages() / 2;
			...
			nr_reclaimed = rtcc_reclaim_pages(nr_to_reclaim,
							  swappiness,
							  &nr_swapped);
			nr_krtccd_swapped += nr_swapped;

			pr_info("%s: reclaimed %ld (swapped %ld) pages.\n",
			...
			if (likely(rtcc_boost_mode == 0)) {
				...
				} else if (swappiness <= 1 && get_file_pages()
					<= rtcc_minfree[RTCC_GRADE_NUM - 2]) {
					/*
					*If swap space is full and file pages
					*is few, also cancel reclaim
					*/
					atomic_set(&need_to_reclaim, 0);
				}
				...
				rtcc_boost_mode = 0;
				atomic_set(&need_to_reclaim, 0);
				pr_info("%s: swapped %ldMB enough, exit boost mode.\n",
					__func__, get_swapped_pages() / 256);
			} else if (time_after(jiffies, boost_end_jiffy)) {
				rtcc_boost_mode = 0;
				pr_info("%s: time out, swapped %ldMB, exit boost mode.\n",
					__func__, get_swapped_pages() / 256);
			}
...
#if RTCC_DBG
			do_gettimeofday(&tv2);
			dt = tv2.tv_sec * 1000000 + tv2.tv_usec -
				tv1.tv_sec * 1000000 - tv1.tv_usec;
			if (nr_reclaimed) {
				pr_info("%s: cost %ldms, %ldus one page, ",
					__func__, dt / 1000, dt / nr_reclaimed);
				pr_info("expect=%ld, swappiness=%d\n",
...
static void rtcc_dump(void)
{
	...
	pr_info("krtccd_running = %d\n", atomic_read(&krtccd_running));
	pr_info("rtcc_boost_mode = %d\n", rtcc_boost_mode);
	pr_info("rtcc_idle_notify_cnt = %d\n", rtcc_idle_notify_cnt);
	pr_info("rtcc_idle_wakeup_cnt = %d\n", rtcc_idle_wakeup_cnt);
}

/* RTCC triggered by framework code */
static ssize_t rtcc_trigger_store(struct class *class,
				  ...
				  const char *buf, size_t count)
{
	...
	if (err) {
		pr_info("rtcc_trigger_store error %d\n", err);
		return count;
	}
...
static int rtcc_idle_handler(struct notifier_block *nb,
...
#ifdef CONFIG_SMP
	if ((num_online_cpus() <= 2) || rtcc_boost_mode) {
#else
	if (unlikely(idle_cpu(task_cpu(krtccd)) && this_cpu_loadx(3) == 0) ||
	    rtcc_boost_mode) {
#endif
...
static int __init
setup_rtcc_threshold(char *str)
{
	int err = 0;

	if (isdigit(*str))
		err = kstrtouint(str, 0, &rtcc_threshold);
	return err;
}

early_param("rtcc", setup_rtcc_threshold);

unsigned int rtcc_threshold_or_default(void)
{
	unsigned int size;

	size = rtcc_threshold;

	return size ? size : (RTCC_ENABLE_THRESHOLD);
}

static int __init rtcc_init(void)
{
	unsigned int threshold;
	unsigned long totolram_size;

	threshold = rtcc_threshold_or_default();
	totolram_size = (totalram_pages << PAGE_SHIFT) >> 20;

	if (totolram_size > threshold) {
		is_rtcc = false;
		pr_info("%s: MemTotal: %8lu MB > Threshold: %4u MB.Rtcc won't work\n)",
			__func__, totolram_size, threshold);
		return 0;
	}
	is_rtcc = true;
...
module_param_array_named(grade, rtcc_grade, uint, &rtcc_grade_size,
			 S_IRUGO | S_IWUSR);
module_param_array_named(minfree, rtcc_minfree, uint, &rtcc_grade_size,
			 S_IRUGO | S_IWUSR);
