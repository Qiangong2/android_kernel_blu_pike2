			   ...
			   int *other_file_p, struct shrink_control *sc)
{
	...
	other_free = global_page_state(NR_FREE_PAGES);
	other_file_orig = global_page_state(NR_FILE_PAGES) -
						global_page_state(NR_SHMEM) -
						total_swapcache_pages();
	...
	other_file = other_file_orig - global_page_state(NR_UNEVICTABLE) +
			unevictable_anon;
	if (other_file < 0)
		other_file = 0;
...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
	...
	int selected_antutu_taskswap = 0;
	...
	for_each_process(tsk) {
		...
		if (!selected_antutu &&
			...
			selected_antutu_taskswap = taskswap;
			continue;
		}
		selected = p;
		...
		lowmem_print(2, "select '%s' (%d), adj %hd, size %d:%d, to kill\n",
			     p->comm, p->pid, oom_score_adj, tasksize, taskswap);
	}
	/* workaround for antutu:
	...
	if (selected && selected_antutu && !has_antutu_3D) {
		if (selected_antutu_adj > selected_oom_score_adj ||
			...
			selected_taskswap = selected_antutu_taskswap;
	...
	if (selected) {
		...
		lowmem_print(1, "Killing '%s' (%d:%d), adj %hd,\n"
			"   to free %ldkB:%ldkB on behalf of '%s' (%d) because\n"
			...
			"   cache_orig is %ldkB\n",
			     ...
			     selected_taskswap * (long)(PAGE_SIZE / 1024),
