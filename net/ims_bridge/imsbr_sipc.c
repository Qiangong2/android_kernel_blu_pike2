 ...
 */
static unsigned int volte_video_apsk;
...
static int volte_video_apsk_set(const char *val, struct kernel_param *kp)
{
	...
	err = kstrtouint(val, 0, &enable);
	if (err) {
		pr_err("%s is invalid, err=%d\n", val, err);
		return err;
	}
	enable = !!enable;

	if (volte_video_apsk != enable) {
		volte_video_apsk = enable;

		pr_info("ltevideo-apsk change to %d\n", enable);
