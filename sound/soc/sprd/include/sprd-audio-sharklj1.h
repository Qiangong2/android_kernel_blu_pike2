...
#ifndef __SPRD_AUDIO_SHARKLj1_H
#define __SPRD_AUDIO_SHARKLj1_H
...
#define PROT_VBC_TO_AP 0x6
/* when switch to wtlcp ap can access */
#define PROT_VBC_TO_WTLCP 0x4
#define PROT_VBC_TO_PUBCP 0x3
					...
					int vbc_idx_max)
{
	...
	case VBC_NO_CHANGE:
		...
		case PROT_VBC_TO_WTLCP:
			return VBC_TO_WTLCP_CTRL;
		case PROT_VBC_TO_PUBCP:
...
static inline int arch_audio_codec_digital_enable(void)
{
	...
	ret = anlg_phy_g_reg_set(REG_ANLG_PHY_G1_ANALOG_BB_TOP_ANA_BB_SINE_CTRL,
		BIT_ANLG_PHY_G1_ANALOG_BB_TOP_SINDRV_ENA);
	if (ret != 0)
...
static inline int arch_audio_codec_digital_disable(void)
{
	...
	ret = anlg_phy_g_reg_clr(REG_ANLG_PHY_G1_ANALOG_BB_TOP_ANA_BB_SINE_CTRL,
		BIT_ANLG_PHY_G1_ANALOG_BB_TOP_SINDRV_ENA);
	if (ret != 0)
		pr_err("%s set failed", __func__);
