...
static void headset_fast_charge(struct sprd_headset *hdst)
{
	...
	queue_work(hdst->reg_dump_work_q, &hdst->fc_work);
}
...
static void headset_detect_all_work_func(struct work_struct *work)
{
	...
	if (hdst->re_detect == true)
		gpio_detect_value = gpio_get_value(pdata->gpios[HDST_GPIO_DET_ALL]);

	pr_info("plug_state_current=%d,plug_stat_last=%d,re_detect=%d\n",plug_state_current,
		hdst->plug_stat_last,hdst->re_detect);
	...
	if ((1 == plug_state_current && 0 == hdst->plug_stat_last ) ||
	  (hdst->re_detect == true && 1 == gpio_detect_value)) {
		...
		case HEADSET_4POLE_NOT_NORMAL:
			...
			//break;
		case HEADSET_NO_MIC:
			pr_info("headset_type = %d (HEADSET_NO_MIC) times_1=%d\n",
				headset_type,times_1);
			if (times_1 < 5) {
				queue_delayed_work(hdst->det_all_work_q,
...
static irqreturn_t headset_detect_all_irq_handler(int irq, void *dev)
{
	...
	pr_info("headset_insert_all_irq_handler in\n");
	headset_irq_button_enable(0, hdst->irq_button);
	headset_irq_detect_all_enable(0, hdst->irq_detect_all);
	wake_lock_timeout(&hdst->det_all_wakelock, msecs_to_jiffies(2000));
	hdst->gpio_det_val_last =
		gpio_get_value(pdata->gpios[HDST_GPIO_DET_ALL]);
	headset_reg_read(ANA_STS7, &val);
	pr_info("headset_detect_all_irq_handler: IRQ_%d(GPIO_%d)(insert all) = %d, ANA_STS7 = 0x%08X\n",
	...
	*/
	...
	pr_info("headset_insert_all_irq_handler out ret %d\n", ret);
	return IRQ_HANDLED;
}
...
int sprd_headset_soc_probe(struct snd_soc_codec *codec)
{
	...
	INIT_WORK(&hdst->fc_work, headset_fc_work_func);

	wake_lock_init(&hdst->det_all_wakelock,
...
static void headset_adc_cal_from_efuse(void)
{
	...
	test[0] = sprd_pmic_efuse_bits_read(DELTA1_BLOCK20*BLK_WIDTH, BITCOUNT);
	test[1] = sprd_pmic_efuse_bits_read(DELTA2_BLOCK22*BLK_WIDTH, BITCOUNT);
