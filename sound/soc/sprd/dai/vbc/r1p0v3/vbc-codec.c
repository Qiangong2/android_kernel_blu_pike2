	...
	struct vbc_codec_priv *vbc_codec)
{
	...
	val_step_vale = BITS_RF_FM_MUTE_DG_STP(vbc_codec->dgmixerstep);
...
static const struct soc_enum vbc_sys_iis_enum[SYS_IIS_MAX - SYS_IIS_START] = {
	SPRD_VBC_ENUM(SYS_IIS0, 12, sys_iis_sel_txt),
	SPRD_VBC_ENUM(SYS_IIS1, 12, sys_iis_sel_txt),
	SPRD_VBC_ENUM(SYS_IIS2, 12, sys_iis_sel_txt),
	SPRD_VBC_ENUM(SYS_IIS3, 12, sys_iis_sel_txt),
};
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (reg == SPRD_VBC_MUX_IDX(SPRD_VBC_AD_IISMUX)) {
		mutex_lock(&vbc_codec->ad01_mutex);
		...
		mutex_unlock(&vbc_codec->ad01_mutex);
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret < profile_max) {
		p_eq_setting->now_profile[vbc_eq_idx] = ret;
...
static int audio_load_firmware_data(struct firmware *fw, char *firmware_path)
{
	...
	do {
		read_len = kernel_read(file, 0, buf, size);
		if (read_len > 0) {
			size -= read_len;
			buf += read_len;
		}
...
static int vbc_mixerdgstep_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct vbc_codec_priv *vbc_codec =
			codec_kcontrol_to_codec_drvdata(kcontrol);

	ucontrol->value.integer.value[0] = vbc_codec->dgmixerstep;

	return 0;
}

static int vbc_mixerdgstep_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct vbc_codec_priv *vbc_codec =
	    codec_kcontrol_to_codec_drvdata(kcontrol);

	vbc_codec->dgmixerstep = ucontrol->value.integer.value[0];
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	vbc_da01_set_dgmixer_step(vbc_codec->dgmixerstep);
	if (id == VBC_MIXERDG_DAC01) {
...
static const struct snd_kcontrol_new vbc_codec_snd_controls[] = {
	...
	SOC_DOUBLE_R_EXT("VBC DAC23 MIXERDG",
		...
		vbc_mixerdg_get,
		vbc_mixerdg_put),
	SOC_SINGLE_EXT("VBC MIXERDG STEP", SND_SOC_NOPM,
				   -1, 0x1FFF, 0, vbc_mixerdgstep_get,
				   vbc_mixerdgstep_put),
...
static int sprd_vbc_codec_probe(struct platform_device *pdev)
{
	...
	/*
	 * all dg setp use the same dgmixerstep, include
	 * da01 dgmixer, da23 dgmixer, fm mute smooth.
	 */
	vbc_codec->dgmixerstep = 0x1;
