...
struct sprd_vbc_priv {
	...
	void (*set_watermark)(uint32_t full, uint32_t empty);
...
static void vbc_dac23_mixto_dac01(int32_t enable,
		       ...
		       struct snd_soc_dai *dai)
{
	...
	pr_info("%s vbc_idx(%d) stream(%d) daiId(%d) VBC(%s) dfm.hw_rate:%d, dfm.sample_rate:%d\n",
		__func__, vbc_idx, substream->stream, dai->id,
		vbc_get_name(vbc_idx), dfm.hw_rate, dfm.sample_rate);
	...
	if (dfm.sample_rate != 0) {
		...
		vbc_idx == VBC_PLAYBACK23 ||
		(vbc_idx == VBC_CAPTURE01 &&
		vbc_codec->ad01_to_fm == true)) {
			pr_info("%s dfm.sample_rate=%d\n",
			    __func__, dfm.sample_rate);
			ret =
...
static void do_vbc_adc_src(int enable, struct vbc_codec_priv *vbc_codec,
			   int vbc_idx)
{
	int32_t vbc_ad_src_idx;
	int32_t adc_src_rate;
...
static int vbc_parse_dt(struct platform_device *pdev)
{
	...
	if (!ret) {
		g_clk_status_addr = devm_ioremap_nocache(&pdev->dev,
					(resource_size_t)val[0],
					(resource_size_t)val[1]);
		if (g_clk_status_addr == NULL) {
			pr_err("%s %d failed\n", __func__, __LINE__);
			return -ENXIO;
		}
	}
...
#if defined(WW6_SND_SOC_SPRD_VBC_LR_INVERT)
/*pike2.dtsi is default set vbc_codec->vbc_iis_lr_invert[0]=0;*/
	vbc_codec->vbc_iis_lr_invert[0]=1;/*add for left and right sound channel invert*/
#endif
...
static int vbc_drv_probe(struct platform_device *pdev)
{
	...
	mutex_init(&vbc_codec->ad01_mutex);
